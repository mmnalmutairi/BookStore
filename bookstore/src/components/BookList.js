...
const BookList = (bookStore) => {
    ...
    const filteredBooks = observer.filter(book => book.name.toLowerCase().includes(query)).map((book) => (
        <Booksitem book={book} setBook={bookStore.setBook} BookDelete={bookStore.BookDelete} />
    ));
    return (
        <div>
